...
import { CreateSedeDto, SedeDto, SedeMinDto } from 'src/app/interfaces/sede.interface';
 ...
 */
  deleteSede(sedeId: number): Observable<{ message: string }> {
    return this.http.delete<{ message: string }>(`${environment.apiUrl}/sede/${sedeId}/`);
      ...
      /**
     * Obtiene la lista de sedes en formato resumido.
     * @returns Observable con la lista de compañías.
     */
    // Método para obtener lista resumida de sedes
  getSedesMin(): Observable<SedeMinDto[]> {
    return this.http.get<SedeMinDto[]>(
      `${environment.apiUrl}/sede/list_min`
    );
  }
}
